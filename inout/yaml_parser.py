...
def validate_schema(data: Dict[str, Any], schema: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Args:
        ...
        schema: A Cerberus schema definition.

    Returns:
        The validated document.

    Raises:
        RFSimError: If the data fails validation.
    """
    validator = Validator(schema)
    if not validator.validate(data):
        logger.error("YAML schema validation errors: %s", validator.errors)
        raise RFSimError("YAML schema validation failed: " + str(validator.errors))
    return validator.document


def parse_netlist(yaml_file: str) -> Circuit:
    """
    Parse the YAML netlist and create a Circuit instance.

    Args:
        yaml_file: File path for the YAML netlist.

    Returns:
        A Circuit object populated with components, connections, and external ports.

    Raises:
        RFSimError: For validation issues or port connection errors.
    """
    ...
    # Process parameters (default to empty dict) and external ports
    circuit.parameters = resolve_all_parameters(data.get("parameters", {}))
    ext_ports = data.get("external_ports", [])
    circuit.external_ports = {
        entry["name"]: Port(entry["name"], index=0,
                            impedance=create_impedance_model_from_config(
                                entry.get("impedance", {"type": "fixed", "value": "50"})
                            ))
        for entry in ext_ports
    }

    # Create components from the netlist
    for comp in data.get("components", []):
        ...
        comp_params = merge_params(circuit.parameters, comp.get("params", {}))
        try:
    ...
    # Create connections between ports and nodes
    for conn in data.get("connections", []):
        ...
        comp_id, port_name = port_ref.split(".", 1)
        try:
    ...
    # Mark the ground node if it exists
    if (gnd_node := circuit.topology_manager.nodes.get("gnd")):
        gnd_node.is_ground = True
...
def parse_sweep_config(yaml_file: str) -> Dict[str, Any]:
    ...
    Args:
        yaml_file: File path for the sweep configuration.

    Returns:
        The validated sweep configuration as a dictionary.
    """
    with open(yaml_file, 'r') as f:
        data = yaml.safe_load(f)
    return validate_schema(data, SWEEP_SCHEMA)
